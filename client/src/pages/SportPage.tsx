...
export function SportPage() {
  ...
  const sportData = getSportBySlug(sport ?? "");
    ...
    return (
      <div className="p-8">
        <EmptyState
          ...
          description="This sport doesn't exist or has no events."
        />
      </div>
    );
  }
  ...
  return (
    <div className="p-4 lg:p-6 space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-slate-400 text-sm">
            {sportData.count} events available
          </p>
        </div>
      </div>
      ...
      {matches.length === 0 && (
        <EmptyState
          icon={sportData.icon}
          title={`No ${sportData.name} events`}
          description="Check back soon for upcoming events in this sport."
        />
      )}
    </div>
  );
