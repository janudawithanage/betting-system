...
export function ResultsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (dateFilter === "Today") {
      ...
      results = results.filter(
        (r) => new Date(r.date).toDateString() === yesterday,
      );
    } else if (dateFilter === "Last 3 days") {
      results = results.filter(
        (r) => now - new Date(r.date).getTime() <= 3 * oneDay,
      );
    ...
    if (search) {
      ...
      results = results.filter(
        (r) =>
          r.homeTeam.name.toLowerCase().includes(q) ||
          r.awayTeam.name.toLowerCase().includes(q),
      );
    }
    return results;
  }, [search, sportFilter, dateFilter]);
    ...
    <div className="p-4 lg:p-6 space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-slate-400">
            Recent match results across all sports
          </p>
        </div>
      </div>
      ...
      {filtered.length > 0 ? (
        <div className="space-y-2">
          {filtered.map((result) => {
            ...
            return (
              <div
                key={result.id}
                className="card p-4 hover:border-bg-hover transition-all"
              >
                {/* League */}
                <p className="text-xs text-slate-500 mb-3">
                  {league?.name} • {formatMatchDate(result.date)}
                </p>
                  ...
                  <div className="flex-shrink-0 text-center">
                    ...
                    {result.scoreHomeHT !== undefined && (
                      <p className="text-xs text-slate-600 mt-0.5">
                        HT: {result.scoreHomeHT}-{result.scoreAwayHT}
                      </p>
                    )}
        ...
        <EmptyState
          ...
          description="Try expanding your filters or search term."
        />
      )}
    </div>
  );
