...
export function ResultsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (dateFilter === 'Today') {
      ...
      results = results.filter((r) => new Date(r.date).toDateString() === today);
    } else if (dateFilter === 'Yesterday') {
      const yesterday = new Date(now - oneDay).toDateString();
      results = results.filter((r) => new Date(r.date).toDateString() === yesterday);
    } else if (dateFilter === 'Last 3 days') {
      results = results.filter((r) => now - new Date(r.date).getTime() <= 3 * oneDay);
    } else if (dateFilter === 'This week') {
      results = results.filter((r) => now - new Date(r.date).getTime() <= 7 * oneDay);
    }
