...
export function FavoritesPage() {
  const { user, isLoggedIn, toggleFavoriteSport } = useAuthStore();

  if (!isLoggedIn) {
    return (
      <div className="p-8 text-center space-y-4">
        <p className="text-2xl">⭐</p>
        <h2 className="text-xl font-bold text-white">Sign in to see your favourites</h2>
  ...
  return (
    <div className="p-4 lg:p-6 space-y-6">
      ...
      {/* Favourite sports */}
      <section>
        <h2 className="font-semibold text-slate-300 mb-3">Favourite Sports</h2>
        {favSports.length > 0 ? (
          ...
          <div className="card p-6 text-center text-slate-500 text-sm">
            No favourite sports yet. Click the ⭐ next to any sport to add it.
          </div>
        )}
      </section>
